...
private:
	...
	static string _ConvertBookRecordToLine(clsBook BookRecord,string Separator = "#//#")
	{
		...
		BookDataLine += to_string(BookRecord.BookPrice);
		return BookDataLine;
	}
	static clsBook _ConvertLineToBookRecord(string Line ,string Separator = "#//#")
	{
		...
		return clsBook(enMode::eUpdateMode,vBookData[0], vBookData[1], vBookData[2], stod(vBookData[3]));
	}
	...
	{
		return clsBook(enMode::eEmptyMode,"", "", "", 0);
	}
...
public:
	...
	clsBook(enMode Mode,string BookID, string BookName, string BookWriter, short BookPrice)
	{
		_Mode = Mode;
		_BookID = BookID;
		_BookName = BookName;
